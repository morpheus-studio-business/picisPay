...
import { filterVoucherBelanjaProducts, Product } from '@/lib/product-categories';
...
export default function VoucherOtherPage() {
    ...
    const fetchAllProducts = async () => {
        try {
            ...
            if (data.success && Array.isArray(data.data)) {
                // Use centralized filter from lib/product-categories
                const otherVouchers = filterVoucherBelanjaProducts(data.data);
                setAllProducts(otherVouchers);
            }
        } catch (error) {
        ...
        try {
            const res = await fetch('/api/digiflazz/transaction', {
                ...
                body: JSON.stringify({
                    ...
                    customer_no: customerNo || 'VOUCHER', // Default to VOUCHER if no input needed (or user input)
        ...
        return (
            <div className="min-h-screen bg-black text-white pb-24 font-sans">
                <header className="sticky top-0 z-20 bg-black/80 backdrop-blur-md border-b border-white/5 px-6 pt-8 pb-4 flex items-center gap-4">
                    ...
                    <h1 className="text-lg font-bold">Voucher Lainnya</h1>
                </header>

                <div className="px-6 py-6">
                    {availableBrands.length === 0 ? (
                        <div className="text-center py-12">
                            <p className="text-neutral-500 text-sm">Tidak ada voucher yang tersedia saat ini</p>
                        </div>
                    ) : (
                        <>
                            <h3 className="text-xs font-bold uppercase tracking-widest text-neutral-500 mb-4">
                                Pilih Kategori ({availableBrands.length} tersedia)
                            </h3>
                            <div className="grid grid-cols-3 gap-3">
                                {availableBrands.map((brand) => (
                                    ...
                                    >
                                        <div className="w-10 h-10 rounded-full bg-orange-600/20 flex items-center justify-center text-sm font-bold text-orange-500">
                        ...
                        >
                            ...
                            <div className="space-y-4 mb-6">
                                <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-4">
                                    <p className="text-xs text-amber-400">⚠️ Setelah pembayaran berhasil, Anda akan menerima <strong>KODE VOUCHER</strong> via notifikasi. Redeem kode tersebut di merchant terkait.</p>
                                </div>
