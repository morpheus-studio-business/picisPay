...
import { ChevronLeft, Loader2, Clock, X, Phone } from 'lucide-react';
    ...
    const fetchAllProducts = async () => {
        try {
            ...
            if (data.success && Array.isArray(data.data)) {
                const masaAktifProducts = data.data.filter(
                    (p: Product) =>
                        (p.category.toLowerCase().includes('masa aktif') ||
                            p.product_name.toLowerCase().includes('masa aktif')) &&
                        p.buyer_product_status &&
                        p.seller_product_status
                );
                setAllProducts(masaAktifProducts);
            }
        } catch (error) {
        ...
        return (
            <div className="min-h-screen bg-black text-white pb-24 font-sans">
                <header className="sticky top-0 z-20 bg-black/80 backdrop-blur-md border-b border-white/5 px-6 pt-8 pb-4 flex items-center gap-4">
                    ...
                    <h1 className="text-lg font-bold">Masa Aktif</h1>
                </header>

                <div className="px-6 py-6">
                    {availableBrands.length === 0 ? (
                        <div className="text-center py-12">
                            <p className="text-neutral-500 text-sm">Tidak ada produk masa aktif yang tersedia saat ini</p>
                        </div>
                    ) : (
                        <>
    ...
    return (
        <div className="min-h-screen bg-black text-white pb-24 font-sans">
            <header className="sticky top-0 z-20 bg-black/80 backdrop-blur-md border-b border-white/5 px-6 pt-8 pb-4 flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-lg font-bold">Masa Aktif {selectedBrand.name}</h1>
